...
- **Reports Layout & Rollover Engine Improvements**: Reordered columns on the Reports page category performance table to match Category, Spent, Avgg monthly, and Current balance, removing the overall annual budgeted column to clean up visual density. Pre-populated the Rollover Engine sidebar inputs with current calculated surpluses (defaulting negative balances/deficits to $0) and updated the backend settings action (`closeYearAndStartNext`) to enforce `rollover = max(0, surplus)` to protect against negative deficit rollovers into the next year.
